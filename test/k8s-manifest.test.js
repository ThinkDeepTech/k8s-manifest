...
describe('k8s-manifest', () => {
  describe('k8sManifest', () => {
                                ...
                                spec:
                                    containers:
                                        - name: "data-collector"
                                          ...
                                          envFrom:
                                          - secretRef:
                                              name: "deep-microservice-collection-secret"
                                    serviceAccountName: "secret-accessor-service-account"
                                    restartPolicy: "Never"
                                    imagePullSecrets:
                                        - name: "docker-secret"
            `;
    ...
    it('should throw an error if a recognized kind is not supplied', () => {
      ...
      try {
        ...
        expect(e.message.toString()).to.include(
          `wasn't found in the k8s client library. Are you sure you supplied an accepted kind?`
        );
      }
    });
  });
  ...
  describe('objectify', () => {
    ...
    it('should correctly convert a k8s client object to a javascript object', () => {
      ...
      expect(obj.spec.containers[0].name).to.equal(
        parsedYaml.spec.containers[0].name
      );
    });
  });
  ...
  describe('k8sClientObject', () => {
    it('should correctly map object types', () => {
      ...
      const subject = k8sClientObject('object', configuration);

      expect(Object.keys(configuration).length).to.equal(
        Object.keys(subject).length
      );
      expect(subject.first).to.equal(configuration.first);
      expect(subject.second).to.equal(configuration.second);
      expect(subject.third).to.equal(configuration.third);
    });

    it('should correctly map date types', () => {
      const configuration = '2022-03-08T15:46:22Z';

      const subject = k8sClientObject('date', configuration);

      expect(subject.toISOString()).to.contain('2022-03-08T15:46:22');
  ...
  describe('container mapping', () => {
    ...
    it('should create a container array of k8s client containers', () => {
      ...
      expect(subject.spec.containers[0].constructor.name).to.include(
        'Container'
      );
    });
    ...
    it('should correctly map startup probe', () => {
      ...
      expect(Array.isArray(subject.spec.containers)).to.equal(true);
      expect(
        subject.spec.containers[0].startupProbe.constructor.name
      ).to.include('Probe');
      expect(subject.spec.containers[0].startupProbe.failureThreshold).to.equal(
        30
      );
      expect(subject.spec.containers[0].startupProbe.periodSeconds).to.equal(
        10
      );
    });
    ...
    it('should correctly map liveness probe', () => {
      ...
      expect(Array.isArray(subject.spec.containers)).to.equal(true);
      expect(
        subject.spec.containers[0].livenessProbe.constructor.name
      ).to.include('Probe');
      expect(
        subject.spec.containers[0].livenessProbe.failureThreshold
      ).to.equal(1);
      expect(subject.spec.containers[0].livenessProbe.periodSeconds).to.equal(
        10
      );
    });
    ...
    it('should correctly map http action', () => {
      ...
      expect(Array.isArray(subject.spec.containers)).to.equal(true);
      expect(
        subject.spec.containers[0].startupProbe.httpGet.constructor.name
      ).to.include('HTTPGetAction');
      expect(subject.spec.containers[0].startupProbe.httpGet.path).to.equal(
        '/healthz'
      );
      expect(subject.spec.containers[0].startupProbe.httpGet.port).to.equal(80);
      expect(subject.spec.containers[0].livenessProbe.httpGet.port).to.equal(
        'liveness-port'
      );
    });
    ...
    it('should correctly map ports', () => {
      ...
      expect(subject.spec.containers[0].ports[0].name).to.equal(
        'liveness-port'
      );
    ...
    it('should correctly map resources', () => {
      ...
      expect(subject.spec.containers[0].resources.constructor.name).to.include(
        'ResourceRequirements'
      );
      expect(subject.spec.containers[0].resources.requests.memory).to.equal(
        '64Mi'
      );
      expect(subject.spec.containers[0].resources.requests.cpu).to.equal(
        '250m'
      );
      expect(subject.spec.containers[0].resources.limits.memory).to.equal(
        '128Mi'
      );
    ...
    it('should correctly map env', () => {
      ...
      expect(subject.spec.containers[0].env[0].constructor.name).to.include(
        'EnvVar'
      );
      expect(subject.spec.containers[0].env[0].name).to.equal(
        parsedYaml.spec.containers[0].env[0].name
      );
      expect(subject.spec.containers[0].env[1].name).to.equal(
        parsedYaml.spec.containers[0].env[1].name
      );
      expect(subject.spec.containers[0].env[2].name).to.equal(
        parsedYaml.spec.containers[0].env[2].name
      );
      expect(subject.spec.containers[0].env[0].value).to.equal(
        parsedYaml.spec.containers[0].env[0].value
      );
      expect(subject.spec.containers[0].env[1].value).to.equal(
        parsedYaml.spec.containers[0].env[1].value
      );
      expect(subject.spec.containers[0].env[2].value).to.equal(
        parsedYaml.spec.containers[0].env[2].value
      );
    ...
    it('should correctly map envFrom', () => {
      ...
      expect(envFrom[0][configMapKey].constructor.name).to.include(
        'ConfigMapEnvSource'
      );
      ...
      expect(envFrom[1][secretRefKey].constructor.name).to.include(
        'SecretEnvSource'
      );
  ...
  describe('pod mapping', () => {
    ...
    it('should set the security context', () => {
      ...
      expect(subject.spec.securityContext.constructor.name).to.include(
        'SecurityContext'
      );
    });
    ...
    it('should set the image pull secrets', () => {
      ...
      expect(subject.spec.imagePullSecrets[0].constructor.name).to.include(
  ...
  describe('job mapping', () => {
    ...
    it('should correctly map pod template', () => {
      ...
      expect(subject.spec.template.constructor.name).to.include(
        'PodTemplateSpec'
      );
    });
    ...
    it('should correctly map pod template metadata', () => {
      ...
      expect(subject.spec.template.metadata.constructor.name).to.include(
        'ObjectMeta'
      );
      expect(subject.spec.template.metadata.labels['controller-uid']).to.equal(
        '50453798-481c-4381-8561-8bacf22b9444'
      );
      expect(subject.spec.template.metadata.labels['job-name']).to.equal(
        'fetch-tweets-apple-business'
      );
    });
    ...
    it('should correctly set selector match expressions', () => {
      ...
      expect(Array.isArray(subject.spec.selector.matchExpressions)).to.equal(
        true
      );
      expect(
        subject.spec.selector.matchExpressions[0].constructor.name
      ).to.include('LabelSelectorRequirement');
    ...
    it('should correctly set selector match labels', () => {
      ...
      expect(subject.spec.selector.matchLabels['controller-uid']).to.equal(
        '50453798-481c-4381-8561-8bacf22b9444'
      );
    });
    ...
    it('should correctly set selector', () => {
      ...
      expect(subject.spec.selector.constructor.name).to.include(
        'LabelSelector'
      );
    });
  ...
  describe('cron job mapping', () => {
    ...
    it('should create a k8s client job template', () => {
      ...
      expect(subject.spec.jobTemplate.constructor.name).to.include(
        'JobTemplateSpec'
      );
    });
    ...
    it('should create a k8s client job spec', () => {
      ...
      expect(subject.spec.jobTemplate.spec.constructor.name).to.include(
        'JobSpec'
      );
    });
  });
  ...
  describe('deployment mapping', () => {
    ...
    it('should correctly map deployment spec strategy', () => {
      ...
      expect(subject.spec.strategy.constructor.name).to.include(
        'DeploymentStrategy'
      );
      expect(subject.spec.strategy.type).to.equal('RollingUpdate');
      expect(subject.spec.strategy.rollingUpdate.constructor.name).to.include(
        'RollingUpdateDeployment'
      );
      expect(subject.spec.strategy.rollingUpdate.maxSurge).to.equal('25%');
      expect(subject.spec.strategy.rollingUpdate.maxUnavailable).to.equal(
        '25%'
      );
    });
    ...
    it('should correctly map deployment spec selector', () => {
      ...
      expect(subject.spec.selector.matchLabels.app).to.equal(
  ...
  describe('persistent volume mapping', () => {
          ...
          csi: {
            ...
            volumeAttributes: {
              'storage.kubernetes.io/csiProvisionerIdentity':
                '1643213782115-8081-dobs.csi.digitalocean.com',
            },
    ...
    it('should correctly map persistent volume spec aws elastic block store ', () => {
      ...
      expect(subject.spec.awsElasticBlockStore.constructor.name).to.include(
    ...
    it('should correctly map persistent volume spec azure disk ', () => {
      ...
      expect(subject.spec.azureDisk.constructor.name).to.include(
    ...
    it('should correctly map persistent volume spec azure file', () => {
      ...
      expect(subject.spec.azureFile.constructor.name).to.include(
        'AzureFilePersistentVolumeSource'
      );
      expect(subject.spec.azureFile.secretName).to.equal('secret-name');
      expect(subject.spec.azureFile.secretNamespace).to.equal(
        'secret-namespace'
      );
    ...
    it('should correctly map persistent volume spec cephfs', () => {
      ...
      expect(subject.spec.cephfs.constructor.name).to.include(
      ...
      expect(subject.spec.cephfs.secretRef.constructor.name).to.include(
        'SecretReference'
      );
      expect(subject.spec.cephfs.secretRef.name).to.equal('secret-name');
      expect(subject.spec.cephfs.secretRef.namespace).to.equal(
        'secret-namespace'
      );
    });
    ...
    it('should correctly map persistent volume spec cinder', () => {
      ...
      expect(subject.spec.cinder.constructor.name).to.include(
      ...
      expect(subject.spec.cinder.secretRef.constructor.name).to.include(
        'SecretReference'
      );
      expect(subject.spec.cinder.secretRef.name).to.equal('secret-name');
      expect(subject.spec.cinder.secretRef.namespace).to.equal(
        'secret-namespace'
      );
    });
    ...
    it('should correctly map persistent volume spec claim reference', () => {
      ...
      expect(subject.spec.claimRef.constructor.name).to.include(
        'ObjectReference'
      );
    ...
    it('should correctly map persistent volume spec csi', () => {
      ...
      expect(subject.spec.csi.constructor.name).to.include(
        'CSIPersistentVolumeSource'
      );
      expect(subject.spec.csi.driver).to.equal('dobs.csi.digitalocean.com');
      expect(subject.spec.csi.fsType).to.equal('ext4');
      expect(subject.spec.csi.volumeHandle).to.equal(
        'e72e4b60-9ef6-11ec-b54c-0a58ac14e15c'
      );
      expect(
        subject.spec.csi.volumeAttributes[
          'storage.kubernetes.io/csiProvisionerIdentity'
        ]
      ).to.equal('1643213782115-8081-dobs.csi.digitalocean.com');

      expect(
        subject.spec.csi.controllerExpandSecretRef.constructor.name
      ).to.include('SecretReference');
      expect(subject.spec.csi.controllerExpandSecretRef.name).to.equal(
        'secret-name'
      );
      expect(subject.spec.csi.controllerExpandSecretRef.namespace).to.equal(
        'secret-namespace'
      );

      expect(
        subject.spec.csi.controllerPublishSecretRef.constructor.name
      ).to.include('SecretReference');
      expect(subject.spec.csi.controllerPublishSecretRef.name).to.equal(
        'secret-name'
      );
      expect(subject.spec.csi.controllerPublishSecretRef.namespace).to.equal(
        'secret-namespace'
      );

      expect(subject.spec.csi.nodePublishSecretRef.constructor.name).to.include(
        'SecretReference'
      );
      expect(subject.spec.csi.nodePublishSecretRef.name).to.equal(
        'secret-name'
      );
      expect(subject.spec.csi.nodePublishSecretRef.namespace).to.equal(
        'secret-namespace'
      );

      expect(subject.spec.csi.nodeStageSecretRef.constructor.name).to.include(
        'SecretReference'
      );
      expect(subject.spec.csi.nodeStageSecretRef.name).to.equal('secret-name');
      expect(subject.spec.csi.nodeStageSecretRef.namespace).to.equal(
        'secret-namespace'
      );
    });
    ...
    it('should correctly map persistent volume spec fc', () => {
      ...
      expect(subject.spec.fc.targetWWNs[0]).to.equal(
        parsedYaml.spec.fc.targetWWNs[0]
      );
      expect(subject.spec.fc.targetWWNs[1]).to.equal(
        parsedYaml.spec.fc.targetWWNs[1]
      );
  ...
  describe('persistent volume claim mapping', () => {
    ...
    it('should correctly map persistent volume claim spec', () => {
      ...
      expect(subject.spec.constructor.name).to.include(
    ...
    it('should correctly map persistent volume claim spec access modes', () => {
      ...
      expect(Array.isArray(subject.spec.accessModes)).to.equal(true);
      expect(subject.spec.accessModes[0]).to.equal(
        parsedYaml.spec.accessModes[0]
      );
    });
    ...
    it('should correctly map persistent volume claim spec selector', () => {
      ...
      expect(subject.spec.selector.matchLabels.app).to.equal(
        parsedYaml.spec.selector.matchLabels.app
      );
    });
    ...
    it('should correctly map persistent volume claim spec resources', () => {
      ...
      expect(subject.spec.resources.constructor.name).to.include(
        'ResourceRequirements'
      );
      expect(subject.spec.resources.requests.storage).to.equal(
        parsedYaml.spec.resources.requests.storage
      );
    });
    ...
    it('should correctly map string fields', () => {
      ...
      expect(subject.spec.storageClassName).to.equal(
        parsedYaml.spec.storageClassName
      );
    });
    ...
    it('should correctly map persistent volume claim status', () => {
      ...
      expect(subject.status.constructor.name).to.include(
    ...
    it('should correctly map persistent volume claim status access modes', () => {
      ...
      expect(subject.status.accessModes[0]).to.equal(
        parsedYaml.status.accessModes[0]
      );
    });
  });
  ...
  describe('metadata mapping', () => {
    ...
    it('should correctly map annotations', () => {
      ...
      expect(
        subject.metadata.annotations['deployment.kubernetes.io/revision']
      ).to.equal(1);
      expect(
        subject.metadata.annotations['meta.helm.sh/release-name']
      ).to.equal('v1');
      expect(
        subject.metadata.annotations['meta.helm.sh/release-namespace']
    ...
    it('should correctly map dates', () => {
      ...
      expect(subject.metadata.creationTimestamp.toISOString()).to.include(
        '2022-03-08T15:46:18'
      );
      expect(subject.metadata.creationTimestamp.constructor.name).to.equal(
        'Date'
      );
    });
    ...
    it('should correctly map labels', () => {
      ...
      expect(subject.metadata.labels['app.kubernetes.io/managed-by']).to.equal(
        'Helm'
      );
    });
    ...
    it('should correctly map owner references', () => {
      ...
      expect(subject.metadata.ownerReferences[0].constructor.name).to.include(
        'OwnerReference'
      );
      expect(subject.metadata.ownerReferences[0].apiVersion).to.include(
        'cache.example.com/v1alpha1'
      );
      expect(subject.metadata.ownerReferences[0].kind).to.equal('Memcached');
      expect(subject.metadata.ownerReferences[0].name).to.equal(
        'example-memcached'
      );
      expect(subject.metadata.ownerReferences[0].uid).to.equal(
        'ad834522-d9a5-4841-beac-991ff3798c00'
      );
    });
  });
  ...
  describe('alpha role mapping', () => {
    ...
    it('should correctly map rules', () => {
      ...
      expect(subject.rules[0].apiGroups[0]).to.equal(
        parsedYaml.rules[0].apiGroups[0]
      );
      expect(subject.rules[0].resources[0]).to.equal(
        parsedYaml.rules[0].resources[0]
      );
  ...
  describe('objectPrefix', () => {
    ...
    it('should correctly map versions that have multiple periods before .k8s.io', () => {
      expect(objectPrefix('flowcontrol.apiserver.k8s.io/v1beta1')).to.equal(
